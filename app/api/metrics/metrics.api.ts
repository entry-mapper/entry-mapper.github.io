...
export const getMetricValuesForCountries = async (
  token: string,
  metric_ids: number[],
  country_ids: number[],
): Promise<CountryMetricValues[]> => {
  try {
    ...
    const countryMetricValues: CountryMetricValues[] = response?.map(
      (metricValue: CountryMetricValues) => {
        return {
